...
- Raw **scraper output** (`sources/data/**/*.md`) is verbose and repetitive. Agents that **modify prompts** need a **compact, structured context artifact** derived from that source text.
...
- **Primary goals**
  ...
  - **Deduplicate** across inputs so the **aggregated context** has **unique headers and unique bullets** at the level of “same prompt method / same rule.”
  - Produce **per-method context files** split by method header (one file per method), with filenames derived from the normalized header name.
- **Secondary goals**
...
- As a **user**, I want **deduplicated summaries** so the **context artifact** contains **only unique content** related to each **prompt method** (no near-duplicate sections).
- As a **user**, I want each prompt method to be available as a **separate file named from the method header** so downstream UI/agent selection can target one method directly.
...
- Point pipeline at `sources/data` (scraped **`.md` only**), run generation, and **merge new summaries into the latest** canonical file ``sources/data/context/prompt_methods_context.csv`` (dedupe after merge).
- From merged sections, write **split method files** under a deterministic folder (e.g. ``sources/data/context/methods/<normalized_method_name>.md``), one file per method header.
...
- Re-runs **append/merge** into canonical context; prior unique content is preserved subject to dedupe rules (exact + optional LM-assisted), and split method files are regenerated/idempotently updated.
...
- **Split-file acceptance:** one output file exists per method header with filename derived from the header and containing that method's rule bullets.
...
- [x] **Output location:** canonical **merge-latest** file ``sources/data/context/prompt_methods_context.csv`` under data root ``sources/data``. **prompt-optimizer-agent** and **prompt-scorer** load this artifact when improving or evaluating prompts.
- [x] **Incremental updates:** **merge into latest** — each pipeline run incorporates new material into canonical context with deduplication (no requirement for monotonic versioned filenames).
- [ ] **Split-file path convention:** exact folder and filename normalization policy for per-method files (slug rules, collisions, max length).
