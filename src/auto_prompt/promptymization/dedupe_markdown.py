...
def exact_dedupe_prompt_method_markdown(markdown: str) -> str:
    ...
    for each header keeps the first occurrence order and **unique** bullet
    lines (exact string match) in first-seen order. Non-bullet body lines under
    a header are merged uniquely in order of appearance.

    Sections without an ATX first line are treated as preamble blocks; identical
    preamble blocks are deduplicated.

    :param markdown: Concatenated Markdown summaries.
    :return: Deduplicated Markdown ending with a single newline.
    """
...
class _SectionMerge:
    """Collect unique non-bullet and bullet lines under one header."""
    ...
    def add_lines(self, body_lines: list[str]) -> None:
        """
        Add lines from a section body, preserving first-seen order for uniqueness.

        :param body_lines: Lines below the header line.
        :return: None.
        """
    ...
    def lines_out(self) -> list[str]:
        """
        Return ordered unique lines for this section.

        :return: Lines to emit under the header.
        """
